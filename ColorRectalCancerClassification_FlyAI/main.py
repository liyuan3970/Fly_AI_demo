...
# 定义命名空间
x = tf.placeholder(tf.float32, shape=[None, 128, 128, 1], name='input_x')
y = tf.placeholder(tf.float32, shape=[None, 8], name='input_y')
...
with tf.name_scope('fc1'):
    ...
    with tf.name_scope('W_fc1'):
        W_fc1 = weight_variable([32 * 32 * 64, 512], name='W_fc1')
    with tf.name_scope('b_fc1'):
        b_fc1 = bias_variable([512], name='b_fc1')
    with tf.name_scope('h_pool2_flat'):
        h_pool2_flat = tf.reshape(h_pool2, [-1, 32 * 32 * 64], name='h_pool2_flat')  # 应该是这一句出现了问题
...
with tf.name_scope('fc2'):
    with tf.name_scope('W_fc2'):
        W_fc2 = weight_variable([512, 8], name='W_fc2')
...
with tf.Session() as sess:
    ...
    for i in range(args.EPOCHS):
        ...
        print("x_train",x_train.shape)#32,1,128,128

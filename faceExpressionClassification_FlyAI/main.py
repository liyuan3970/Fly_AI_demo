...
fc2 = fc(name='fc2', x=fc1, out_channel=7)
prediction = tf.nn.relu(fc2, name="prediction")
y_conv = tf.nn.relu(fc2, name="y_conv")

# 计算交叉熵损失并汇总为标量数据
with tf.name_scope("cross_entropy"):
    cross = tf.losses.softmax_cross_entropy(tf.one_hot(y, depth=7), prediction)
...
with tf.Session() as sess:
    ...
    for i in range(args.EPOCHS):
        ...
        loss = sess.run(cross_entropy, feed_dict={x: x_train, y: y_train, keep_prob: 0.8})
